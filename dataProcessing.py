        ...
        for n in [nuts1, nuts2, nuts3]:
            ...
            if n == nuts1:
                try:
                    ...
                    dataframe, jsonfile = generate_dataframe(('{}/{}/shapefiles/nuts1/{}').format(path, zone, res[0]), ('{}/{}/pollutionNO2/{}').format(path, zone, tif))
                    dataframe['year'] = year
                    dataframe['month'] = month
                    dataframe['polluant'] = 'no2'
                    dataframe.to_csv(('{}_no2_nuts1_{}_{}.csv').format(zone, year, month))
                except:
                    pass
            elif n == nuts2 :
                try:
                    res = [i for i in n if '.shp' in i]
                    dataframe, jsonfile = generate_dataframe(('{}/{}/shapefiles/nuts2/{}').format(path, zone, res[0]), ('{}/{}/pollutionNO2/{}').format(path, zone, tif))
                    dataframe['year'] = year
                    dataframe['month'] = month
                    dataframe['polluant'] = 'no2'
                    dataframe.to_csv(('{}_no2_nuts2_{}_{}.csv').format(zone, year, month))
                except:
                    pass
            else:
                try:
                    res = [i for i in n if '.shp' in i]
                    dataframe, jsonfile = generate_dataframe(('{}/{}/shapefiles/nuts3/{}').format(path, zone, res[0]), ('{}/{}/pollutionNO2/{}').format(path, zone, tif))
                    dataframe['year'] = year
                    dataframe['month'] = month
                    dataframe['polluant'] = 'no2'
                    dataframe.to_csv(('{}_no2_nuts3_{}_{}.csv').format(zone, year, month))
                except:
                    pass
